...
| 82 | SCRUM Backlog Pass (Repo Hygiene + All 5 Sessions + Skill Barrier Audit) — Full backlog tracked in `SCRUM_BACKLOG.md` as six Epics with INVEST-scored stories, all closed. **Repo hygiene:** untracked 1.3GB dead map pack (`Maptool/maps/Public/`), stock demo campaign, a dated backup `.cmpgn`, and tracked `__pycache__`; deleted five stale scratch docs; ran a `git-filter-repo` history rewrite (force-pushed) shrinking `.git` from 2.3GB to 370MB. **Session 3:** fixed a broken script path and a contradictory token note in `chapter03_maptool.qmd`; added missing Tribune Lucius and Cassia OGAS entries; wired Hilde, Aldric, and Arnulf (previously unused `chapter03_npcs.html` cards) into the actual chapter text as a Clue-2 alternate path, a Brutus informant, and the bog-offering-pole caretaker; added a missing Cassia card. **Session 4:** fixed the same script-path bug plus a stale "Maps for This Session" table pointing at a leftover grove/ritual scene instead of the real tunnel sequence, and dropped the matching dead-weight map copies from the build script. **Session 5:** fixed the same script-path bug, a wrong Dice Grimorium folder slug (`ForestLabyrinthRuins` → `LabyrinthRuins`, silently breaking that scene's map copy), wired the purpose-built `s5_mars_confrontation`/`s5_mars_vault*` SVGs into the scene table, fixed a Brutus name typo, removed a duplicated paragraph, and added an OGAS resolution summary. **Sessions 1-2:** lighter pass as expected (both already had a dedicated milestone) -- added the four purpose-built vault SVGs to chapter1.qmd's own map table and cross-referenced Tribune Lucius and Marius Coda in its OGAS table. **Skill barrier audit:** completed the format normalization from Milestone 72 across chapter1/2/5.qmd, roles.qmd, and roman_tactics.qmd (chapter3.qmd/chapter4.qmd were already clean) per `AGENT_TASK_skill_barrier_audit.md`. |
...
- Ch3: `s3_forest_path.html`, `s3_germanic_village.html`, `s3_farbog_crossing.html`, `s3_forest_overview.html`, `s4_sacred_grove.html`, `s4_sacred_grove_overview.html` (sacred grove is Ch3's own Scene 4-5, despite the `s4_` filename prefix)
- Ch4: `s4_fort_siege.html`, `s4_tunnel_armory.html`, `s4_tunnel_choking_hall.html`, `s4_tunnel_elder_stair.html`
...
Live backlog tracking now happens in `SCRUM_BACKLOG.md` (Epics + INVEST stories). All six
epics from the Milestone 82 pass are closed. Add new epics there as they come up rather

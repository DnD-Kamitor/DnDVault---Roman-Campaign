...
- [x] **Milestone 85** — Map and token audit for S0 and S1. S1 reduced to 1 dungeon map (DungeonVol2: stone tile dungeon with central circular altar). S0 uses BridgeCheckpoint for road arrival, Fort_Vindolanda folder for all fort scenes. Token fixes: AnimatedArmor removed (vault redesign); Wight + Ghast added (Bone Chamber and Altar Chamber); CorruptedWorker renamed to MariusCoda_Berserker; named cultists TitusFlavinus and GnaeusPeregrinus added; AeliusRufus dying legionary added; MariusCoda_Laborer added to Fort_Vindolanda for S0 prologue tavern scene. Map table in chapter1.qmd updated. Map table added to gm_session0.qmd.

...
Worsening triggers:
- Two consecutive sessions without a fading reset: +1 stage (mark the first miss session; the second miss triggers the move)
- Three accumulated shadow-curse manifestations (from the corruption divergence section above): +1 stage
- Both conditions in the same session: +1 stage, not +2; stage progression has a ceiling of one per session

Lessening triggers:
- Stages 1-2: three consecutive sessions with the fading mechanic successfully reset each time: -1 stage
- Stage 2+: the grove ritual (Session 4 or as a side mission): -1 stage
- Stage 3+: sensation alone cannot reduce stage; requires ritual or divine intercession
- Stage 4: grove ritual drops to Stage 3; from Stage 3 normal recovery applies
